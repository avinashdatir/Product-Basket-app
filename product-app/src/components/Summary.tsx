...
const Summary = ({ items }: SummaryProps) => {
  ...
  const cheeseItem = items.find(
    (item: any) => item.name === "Cheese"
  );

  if (cheeseItem) {

    const freeCheese = Math.floor(
      cheeseItem.quantity / 2
    );

    totalSavings +=
      freeCheese * cheeseItem.price;
  }

  const soupItem = items.find(
    (item: any) => item.name === "Soup"
  );

  const breadItem = items.find(
    (item: any) => item.name === "Bread"
  );
  ...
  if (soupItem && breadItem) {
    ...
    totalSavings +=
      eligibleBread * (breadItem.price / 2);
  }

  
  const butterItem = items.find(
    (item: any) => item.name === "Butter"
  );
